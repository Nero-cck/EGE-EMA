...
class EMA(nn.Module):
    def __init__(self, channels, factor=8):
        ...
        self.groups = factor
        assert channels // self.groups > 0, "Channel must be divisible by groups"
    ...
    def forward(self, x):
        ...
        x11 = self.softmax(self.agpool(x1).reshape(b * self.groups, -1, 1).permute(0, 2, 1))
        x12 = x2.reshape(b * self.groups, c // self.groups, -1)  # b*g, c//g, hw
        x21 = self.softmax(self.agpool(x2).reshape(b * self.groups, -1, 1).permute(0, 2, 1))
